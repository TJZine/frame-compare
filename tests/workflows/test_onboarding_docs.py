...
def test_vsview_extra_pins_supported_native_windows_media_stack(repo_root: Path) -> None:
    ...
    assert {dependency for dependency in dependencies if dependency.startswith("vsview==")} == {
        "vsview==0.11.0"
    }
    assert "vapoursynth==80" in dependencies
    ...
    assert "package to R80" in native_install
