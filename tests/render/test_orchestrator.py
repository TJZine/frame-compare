...
def test_render_screenshots_vs_loading(tmp_path):
    clips = [Path("vid1.mkv")]
    frames = [10, 20]
    ...
    with patch("frame_compare.vs.loader.DefaultVSLoader") as mock_loader_cls:
        ...
        with patch("frame_compare.render.orchestrator.render_batch") as mock_batch:
            ...
            results = render_screenshots(clips, frames, tmp_path, renderer="vapoursynth")
...
def test_render_screenshots_fallback(tmp_path):
    clips = [Path("vid1.mkv")]
    frames = [10]
    ...
    with patch("frame_compare.vs.loader.DefaultVSLoader") as mock_loader_cls:
        ...
        with patch("frame_compare.render.orchestrator.render_batch") as mock_batch:
            ...
            with capture_logs() as captured:
                results = render_screenshots(clips, frames, tmp_path, renderer="auto")

            assert "vid1" in results
            assert any(log["event"] == "vs_load_failed_falling_back" for log in captured)


def test_render_screenshots_vs_forced_fail_vs_not_found(tmp_path):
    clips = [Path("vid1.mkv")]
    with patch("frame_compare.vs.loader.DefaultVSLoader") as mock_loader_cls:
        mock_loader = mock_loader_cls.return_value
        ...
        with pytest.raises(VapourSynthNotFoundError):
            render_screenshots(clips, [1], tmp_path, renderer="vapoursynth")


def test_render_screenshots_vs_forced_fail_plugin(tmp_path):
    clips = [Path("vid1.mkv")]
    with patch("frame_compare.vs.loader.DefaultVSLoader") as mock_loader_cls:
        mock_loader = mock_loader_cls.return_value
...
def test_render_screenshots_vs_forced_fail_source(tmp_path):
    clips = [Path("vid1.mkv")]
    with patch("frame_compare.vs.loader.DefaultVSLoader") as mock_loader_cls:
        mock_loader = mock_loader_cls.return_value
        mock_loader.load.side_effect = SourceLoadError(Path("vid1.mkv"), "Failed")
...
def test_render_screenshots_vs_forced_fail_unknown(tmp_path):
    clips = [Path("vid1.mkv")]
    with patch("frame_compare.vs.loader.DefaultVSLoader") as mock_loader_cls:
        mock_loader = mock_loader_cls.return_value
...
def test_render_screenshots_fallback_unknown(tmp_path):
    clips = [Path("vid1.mkv")]
    frames = [10]
    ...
    with patch("frame_compare.vs.loader.DefaultVSLoader") as mock_loader_cls:
        ...
        with patch("frame_compare.render.orchestrator.render_batch") as mock_batch:
            ...
            assert "vid1" in results
            assert any(log["event"] == "vs_load_failed_falling_back_unknown" for log in captured)


def test_render_screenshots_overlay_resolution(tmp_path):
    clips = [Path("vid1.mkv")]
    with patch("frame_compare.vs.loader.DefaultVSLoader") as mock_loader_cls:
        mock_loader = mock_loader_cls.return_value
        mock_source = MagicMock()
    ...
    with patch("frame_compare.vs.loader.DefaultVSLoader") as mock_loader_cls:
        ...
        with patch("frame_compare.render.orchestrator.render_batch") as mock_batch:
            mock_batch.return_value = [tmp_path / "1.png"]
            render_screenshots(clips, [1], tmp_path, renderer="auto")
            req = mock_batch.call_args[0][0][0]
            assert req.overlay.resolution == (0, 0)


def test_render_screenshots_dict_order(tmp_path):
    clips = [Path("b.mkv"), Path("a.mkv")]
    frames = [1, 2]
    with patch("frame_compare.render.orchestrator.render_batch") as mock_batch:
        mock_batch.return_value = [Path(f"{i}.png") for i in range(4)]
        results = render_screenshots(clips, frames, tmp_path, renderer="ffmpeg")
...
def test_render_screenshots_output_path(tmp_path):
    clips = [Path("vid1.mkv")]
    frames = [42]
    with patch("frame_compare.render.orchestrator.render_batch") as mock_batch:
        mock_batch.return_value = [tmp_path / "vid1_000042.png"]
        render_screenshots(clips, frames, tmp_path, renderer="ffmpeg")
