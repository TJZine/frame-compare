...
- *2025-11-08:* Hardened CLI tonemap overrides against NaN/Inf, sanitised Legacy filenames with a regex that strips the Windows-illegal characters, resolved the spinner `accent` role to its concrete Rich style, respected explicit colour-range overrides across signal heuristics and post-tonemap metadata, and reinstated screenshot tests using the VapourSynth stub instead of a global skip.
