...
| Native VSView alignment panel | Included | No | No | Experimental (offscreen proof only) | Optional with `frame-compare[vsview]` |
...
contains the selected runtime and native Frame Compare VSView alignment panel, and
provides user-level install, update, backup, rollback, and uninstall commands.
...
- The image is currently built from the repository rather than pulled from a promised
  public registry.
- The default route does not provide the VSView runtime or native alignment panel.
