...
The run service mounts configuration and media read-only while keeping screenshots
and generated output on the host. The first-use wizard keeps
...
(or `/workspace/screenshots/` to `./screenshots/`) and open `report.html` normally.
