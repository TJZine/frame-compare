...
| R6 production acceptance | Partial evidence complete; blocked | `897d40da` | Native macOS and canonical Docker resource/RSS/cancellation gates plus the bounded independent review passed; physical Windows, visible VSView, and independently labeled real-media cells remain blocking. |
| R7 activation | Blocked by incomplete R6 acceptance | — | Physical Windows, visible VSView, real-media, unpatched defaults, and packaged candidate proof required. |
...
- **Blocking acceptance / stop gate:** R6 is not accepted. No native Windows portable host or physical Windows visible-VSView environment is available, and the repository contains no independently labeled real constant-zero, signed-offset, or mismatched mix/cut/track media suitable for the mandatory visual matrix. Docker/offscreen and synthetic fixtures are not substituted for those cells. These are explicit R6 stop conditions, so R7 remains blocked and automatic authority remains held. The next permitted action requires a suitable physical Windows environment and independently labeled real-media inputs; it is not an estimator or architecture change.

#### R6 physical-Windows follow-up — September 19, 2026

- **Candidate and retained evidence:** a physical unmanaged-Windows source-checkout run used Frame Compare 0.6.0 and a checkout inferred from the local reflog as `9401f47177a68cdd9a4202a8680c6d73e099a546`; the run record itself does not embed a commit or dirty-state fact. The candidate contains `8044716f` (`fix(alignment): preserve diagnostic invariants`) and predates `73ecea72` (`fix(cli): show active audio analysis progress`), whose absence affects transient progress presentation rather than estimation. The share-safe scalar record is `tests/fixtures/alignment_oracle/windows-real-media-results.json`; it binds the retained local run artifacts and three CLI captures by SHA-256 without tracking media, absolute media paths, generated VSView scripts, or raw PCM.
- **Observed runtime and execution:** `doctor --json` succeeded on the matching unmanaged-Windows runtime fingerprint with VapourSynth R79/API R4.2, L-SMASH-Works, vs-placebo 2.0.4, the VSView panel, and the selected FFmpeg/ffprobe build available. The CLI visibly loaded three sources, offered a previously human-confirmed zero-offset set, recorded the user's `n`, and performed fresh discovery plus requested-rate verification for both comparisons. Exactly two schema-v2 diagnostics retained `current_attempt`/`observed` evidence; all eight collections completed at their planned endpoints in 11.30–13.28 seconds with no EOF, failure, or cleanup failure. The complete align phase took 270.57 seconds, including native review; no collection approached the 120-second deadline.
- **Decisions and authority:** Comparison 1 produced a held `+0f` candidate from 4 raw/credible/voting/winning windows and 3 independent windows; its fifth late row was a non-credible 473-frame estimate with score 0.0134 and peak ratio 1.0087, so the system correctly exposed `possible_discontinuity` near 01:27:05 without treating that row as a credible contradiction. Comparison 2 produced a held `+0f` candidate from 2 credible/voting/winning windows but no qualifying three-region independent subset, correctly reporting `insufficient_independent_support`. Both hints were visibly labeled provisional and not applied. A later whole-set native action wrote exactly two ordered confirmed `+0f` decisions and the final diagnostics identify `interactive_confirmed_this_run`; no provisional candidate silently gained authority.
- **Physical/UI evidence:** the user described the three files as peer release sources with no known offset and reported the same visible moment at untrimmed frames 5,000 and 10,000 for all three, yielding `reference - comparison = 0f` for both comparisons at those two early points. CLI captures show the three named VSView outputs, both provisional hints, the complete two-decision acceptance, manual final source, the retained discontinuity warning, and successful render/report completion. The captures do not establish panel readiness 3/3, no-prefill/playhead/visited-state behavior, details readability, visible marker bounds, or close-without-save behavior.
- **Disposition and remaining stop gate:** this adds a physical-Windows unmanaged-runtime and one tentative real zero-offset source-set observation, but it does not close R6. The two visual checks are both early; the warning position near untrimmed frame 125,281 plus middle, later, and final shared content remain unchecked. No independently labeled signed-offset or mismatched mix/cut/track pair was supplied. Windows simultaneous parent-plus-active-FFmpeg RSS remains unavailable from a normal run, and no portable-bundle commit identity or Windows resource/lifetime sampler was observed. Both comparisons therefore remain inconclusive for full-program real-media acceptance, no estimator adjustment is supported, R7 remains blocked, and automatic authority remains held.
