...
- **`[audio_alignment]`** – enable flag, reference clip label, VSPreview toggles/mode, prompt reuse, STFT parameters (`sample_rate`, `hop_length`), optional start/duration, `correlation_threshold`, `max_offset_seconds`, offsets filename, and `random_seed`.
