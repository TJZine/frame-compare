...
- [ ] Remove the unused audio-alignment confirmation hook and layout fields: after deleting `confirm_with_screenshots`, `_confirm_alignment_with_screenshots` now auto-confirms but still flows through `AlignmentWorkflow.run` signatures, reporter wiring, and JSON tail/layout (`AudioAlignmentDisplayData.confirmation/preview_paths`). Follow-up should (1) strip the confirmation callback from `AlignmentWorkflow`/`services/factory` and runner wiring, (2) delete `confirmation`/`preview_paths` from `cli_layout.v1.json`, `cli_runtime.AudioAlignmentDisplayData`, and JSON tail assembly, adjusting CLI tests/goldens (`tests/runner/test_audio_alignment_cli.py`, `tests/helpers/runner_env.py`) accordingly, and (3) update docs/DECISIONS/CHANGELOG to note the removal. Expect a small cascade in `runner.format_alignment_output`, VSPreview script seeding, and any downstream consumers using `json_tail["audio_alignment"]["confirmation"]`.
