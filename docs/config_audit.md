...
**Health check.**
- **Geometry/padding:** `mod_crop`, `letterbox_pillarbox_aware`, `auto_letterbox_crop`, `pad_to_canvas`, `center_pad`, and `letterbox_px_tolerance` all terminate in `_plan_geometry` (`src/screenshot.py:1634-1888`) and `_align_letterbox_pillarbox`. Odd-geometry policies (`auto`, `force_full_chroma`, `subsamp_safe`) are enforced per format, with clear warnings when `subsamp_safe` rebalance introduces one-pixel shifts. The new multi-mode auto-letterbox flow is covered by `tests/test_screenshot.py::{test_auto_letterbox_crop,test_auto_letterbox_basic_avoids_weird_scope_pair,test_auto_letterbox_strict_preserves_legacy_scope_behavior}`.
