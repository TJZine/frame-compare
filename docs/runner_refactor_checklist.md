...
### Phase 11.7 – Retry/backoff consolidation (2025‑11‑12)

| Checklist Item | Status | Notes / Next Steps |
| --- | --- | --- |
| Helper module | ☑ | Added `src/frame_compare/net.py` with `build_urllib3_retry`, `default_requests_timeouts`, `httpx_get_json_with_backoff`, and `redact_url_for_logs`, and exported them via `__all__`. |
| slow.pics wiring | ☑ | `src/frame_compare/slowpics.py` now calls `build_urllib3_retry` when constructing its `HTTPAdapter`, keeping the adapter/mount seam for request/session monkeypatches. |
| TMDB wiring | ☑ | `src/tmdb.py::_http_request` now awaits `net.httpx_get_json_with_backoff` so retries/backoff are centralized while the cache/error handling stays unchanged. |
| Verification | ☑ | `PYTEST_DISABLE_PLUGIN_AUTOLOAD=1 .venv/bin/pytest -q`, `.venv/bin/ruff check`, `.venv/bin/pyright --warnings`, and `uv run --no-sync lint-imports --config importlinter.ini` all passed locally after adding `src.frame_compare.net` to the Runner→Core→Modules layer. |
| Residual risks | ⚠️ | The optional VSPreview extra is still required for some slow.pics tests, so keep running the slow.pics/runner suite with the extra installed and preserve the adapter/session seams for future tweaks. |

### Phase 11.8 – Config docs generation (2025‑11‑12)

| Checklist Item | Status | Notes / Next Steps |
| --- | --- | --- |
| Generator CLI | ☑ | `tools/gen_config_docs.py` introspects `src/datatypes.py`, produces `docs/_generated/config_tables.md`, and exposes `--check` for drift detection without mutating the repository. |
| Generated docs | ☑ | `docs/_generated/config_tables.md` is now committed and linked from `docs/README_REFERENCE.md`; the comment at the top signals editors not to edit manually. |
| README link | ☑ | The reference guide now says the tables are generated by `tools/gen_config_docs.py` and points to `docs/_generated/config_tables.md`, so maintainers know where to look for the source of truth. |
| Tests | ☑ | Added `tests/docs/test_config_docs_gen.py` (see `CHANGELOG.md` entry) plus the sentinel run `python3 tools/gen_config_docs.py --check docs/_generated/config_tables.md` to guard `src/datatypes.py` tweaks. |
