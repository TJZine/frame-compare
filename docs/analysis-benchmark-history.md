...
| 2026-07-12 | [`analysis-tier-benchmark-witch-baseline.json`](../generated/analysis-tier-benchmark-witch-baseline.json) | Older-schema `quality` median 232.491s versus `performance` 115.031s, about 2.02x faster. Bright and motion were 10/10; dark was 8/10. | Baseline for the later Witch optimization work. Keep separate from the June run because the schema and implementation context changed. |
| 2026-07-12 | [`analysis-tier-witch-combined-sync.json`](../generated/analysis-tier-witch-combined-sync.json) and [`analysis-tier-witch-combined-concurrent.json`](../generated/analysis-tier-witch-combined-concurrent.json) | Combined PlaneStats traversal reproduced the established luminance and motion top-50 sets (1.0 Jaccard overlap); Spearman correlation was 0.999969 for luminance and 0.960388 for motion. | Implementation evidence only. The artifacts are superseded by the accepted quality implementation and final candidate matrix. |
| 2026-07-13 | [`quality-planestats-candidate-witch-4k-hdr.json`](../generated/quality-planestats-candidate-witch-4k-hdr.json) | Full-resolution PlaneStats median 64.009s versus production NumPy quality 235.086s: 3.67x faster and 72.77% less compute time. Selected frames and top-50 orderings were exact; maximum motion drift was 8.80e-9. | Accepted under the practical-equivalence quality contract, which bounds float drift while requiring exact selections and rankings. SDR and animation/grain evidence remain required for release confidence. |
| 2026-07-13 | [`analysis-decoder-candidates-witch-4k-hdr.json`](../generated/analysis-decoder-candidates-witch-4k-hdr.json) | Dense skip-loop-filter candidate: 65.098s versus the 68.749s PlaneStats candidate, about 1.056x faster. Max-threads variant: 66.801s, about 1.029x faster. Category retention was 100% in this run, but neither approached the 1.5x minimum target. | No separate dense third performance tier was justified by this evidence. |
| 2026-07-13 | [`analysis-all-candidates-witch-4k-hdr.json`](../generated/analysis-all-candidates-witch-4k-hdr.json) | Relative to the report's 23.135s quality reference: normal 25% sparse was 2.075x, normal 12.5% sparse was 3.207x, and 6.25% sparse was 3.735x. The 6.25% modes retained only 60% of motion selections. Skip-loop sparse variants were faster again, but added another approximation. | Narrowed the final decision run to normal-decoder 25% and 12.5% sparse candidates, with animation/grain as the next required corpus. |
| 2026-07-13 | [`analysis-nvidia-cuvid-request-witch-4k-hdr.json`](../generated/analysis-nvidia-cuvid-request-witch-4k-hdr.json) | CUVID-request candidate was 1.204x faster than the report's quality reference and retained 100% of bright, dark, and motion selections. | Diagnostic result only: GPU presence and device-wide utilization did not prove that L-SMASH selected CUVID instead of falling back to software. |
...
- Keep the Witch baseline, accepted quality evidence, latest final sparse
  evidence, latest NVIDIA evidence, and one full candidate matrix until the
  next corpus completes.
- Keep final SDR and animation/grain reports when they are produced; these are
  the cross-corpus release evidence.
- Archive or remove superseded combined traversal reports, dense decoder-only
  reports, and scratch reports after their decision metrics are captured here.
- The older NVIDIA request report is superseded by the final Witch NVIDIA
  report; retain one of them, not both.
...
- The animation/grain corpus has not yet been recorded in this ledger.
- The 12.5% sparse candidate has a 10% bright-category miss on the final Witch
  window even though its sampled ranking was exact; the animation result will
  decide whether that tradeoff is acceptable.
