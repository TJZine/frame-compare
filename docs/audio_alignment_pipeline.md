...
8. **CLI output & JSON tail** – The reporter prints stream selections, estimated offsets, and any warnings. The JSON tail captures reference/target descriptors, per-label offsets, correlation threshold, preview paths, and confirmation state for downstream tooling. When trims are applied or reused from persisted manual edits, the `offsets_frames` map now reflects those applied frame counts rather than the most recent raw measurements. 【F:frame_compare.py†L1332-L1368】【F:frame_compare.py†L1654-L1670】【F:frame_compare.py†L2255-L2281】
