...
- progress bar is still a dot on a solid line when set to fill, Investigate why progress bar color is linked to heading color (CYAN)
...
- when panning while in slider mode to a zoomed area, moving the slider itself sets the alignment back to center and reverts the custom manual zoom of the user trying to view the difference of a specific area zoomed in.
-  use sequential thinking. use context7. use RIPGREP for any searching. Do a thorough analysis of legacy/comp.py and identify any possible regressions in our project vs this original file and any possible improvements to take from it. 2 things of note ive noticed, screenshot progress bar in ours is a regression, the old version shows which encode is being screenshot as well as a total progress bar. 2. Uploaded to slow.pics is exponentially faster in this old version of the script.
