...
## TODO
Adjust vs-preview behavior. Ensure vs-preview opens properly when enabled in the settings. When enabled alongside audio_alignment i want it to function instead as a confirmation tool in place of our old screenshot comparison system where it would write screenshots to check alignment but instead the calculate offset from audio alignment portion shrunk 3 frames closer to 0 then applied when opening the vs-preview so the offset is easier to find. When vs-preview is on and audio alignment is off I want vs-preview to behave like there is nothing known about the video offsets and start with the complete source videos for manual alignment by the user, when alignment is made its applied and the manual offset value is saved for use on future runs of the same sources. Questions: How to run in an interactive shell vs automatic? any way to get vs-preview feature to work if the shell isnt interactive? I want to either still have it run and automatically spawn the vs-preview window to make the comparison, then give confirmation in the CLI or through the vs-preview window itself when they are aligned and the script can continue. Im open to options and suggestions regarding this.
