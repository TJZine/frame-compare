...
class _AnsiColorMapper:
    ...
    def __init__(self, *, no_color: bool) -> None:
        """
        Initialize the color mapper and determine terminal color capability.
        
        Sets self.no_color from the given flag combined with the NO_COLOR environment variable. If coloring is disabled sets self._capability to "none"; otherwise attempts to enable Windows VT processing when appropriate and detects the terminal's color capability, storing the result on self._capability. May modify console state on Windows to enable VT processing.
         
        Parameters:
            no_color (bool): If True, force-disable ANSI color output regardless of environment.
        """
    ...
    def _is_truthy_flag(raw_value: str) -> bool:
        """
        Interpret an environment-style flag string as a boolean indicator of enabled behavior.
        
        Parameters:
            raw_value (str): Raw flag value (for example from an environment variable); whitespace is ignored and comparison is case-insensitive.
        
        Returns:
            true if the value represents an enabled/Truthy flag (non-empty and not one of "0", "false", "no", "off"), false otherwise.
        """
    ...
    def _is_modern_windows_terminal() -> bool:
        """
        Detects whether the current environment appears to be a VT-capable Windows terminal.
        
        Determination is based on observable indicators: running on Windows, presence of the WT_SESSION environment variable, TERM_PROGRAM set to "windows_terminal", or a Windows version greater than 10 (or equal to 10 with build >= 10586). Other platforms always report not modern.
        
        Returns:
            `True` if heuristics indicate a VT-capable Windows console, `False` otherwise.
        """
    ...
    def _detect_capability() -> str:
        """
        Determine terminal color capability based on environment variables and platform heuristics.
        
        Considers the FRAME_COMPARE_FORCE_256_COLOR environment variable, COLORTERM, TERM, and Windows terminal heuristics when deciding capability.
        
        Returns:
            capability (str): "256" if 256-color/truecolor support is likely, "16" otherwise.
        """
    ...
    def apply(self, token: str, text: str) -> str:
        ...
        Parameters:
            token (str): Color/style token name (e.g., "green.bold") used to look up ANSI codes.
            text (str): Text to style; an empty string is returned unchanged.
        
        Returns:
            str: Text wrapped with the token's ANSI SGR sequence and an ANSI reset, or the original `text` if no styling is applied or coloring is disabled.
        """
