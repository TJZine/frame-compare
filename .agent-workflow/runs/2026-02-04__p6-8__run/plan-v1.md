...
- JSON output schema for successful runs beyond existing progress reporter selection (future slice unless SSOT defines an output schema).
...
**Changes:**
- Add missing CLI flag `--force-interactive-alignment` to match SSOT and to drive config override behavior (§4.4.5).
- Replace stub body with:
  - Build a `RunRequest` from CLI args (no hidden defaults beyond Typer defaults).
...
- `overlay` CLI arg maps to `RunRequest.overlay_mode` (naming mismatch in current code).
- `tm_target` CLI arg maps to `RunRequest.tm_target_nits` (naming mismatch in current code).
- `--force-interactive-alignment` must be represented on the request so the orchestration layer can apply config overrides deterministically.
...
**Changes:**
- Add `force_interactive_alignment: bool = False` to `RunRequest` to carry the CLI flag through to orchestration (required to implement §4.4.5 without environment-variable hacks).
- Apply CLI overrides to the loaded config before creating `RunContext`:
  - Build a dict of CLI override values from the `RunRequest`.
  - Call `frame_compare.config.overrides.apply_cli_overrides(preflight.config, cli_args=...)`.
  - Use the overridden config in `RunContext`.
- Use `discover_inputs(...)` per §4.4.6 semantics (error on empty result, stable order).

**Functions to implement (spec-anchored):**
- `execute_run(request: RunRequest, deps: RunDependencies | None = None) -> RunResult`

**Key implementation notes:**
- Override priority must be CLI > config file > defaults (apply overrides after config load, before pipeline phases).
- `--force-interactive-alignment` must imply both:
...
**Changes:**
- Update `discover_inputs` signature to include default patterns list.
...
**Changes:**
- Update `CLI_OVERRIDE_MAP` to match §4.4.5 names and paths:
  - Map `seed` → `analysis.random_seed` (not `random_seed`).
  - Add `force_interactive_alignment` → `audio_alignment.force_interactive` (and implement the implied `audio_alignment.use_vspreview = True` behavior).
...
- Document that it maps to `audio_alignment.force_interactive` and implies `audio_alignment.use_vspreview = True` (as a note/comment in the contract entry).
...
- Replace `test_run_stub_executes` with a deterministic unit test that monkeypatches `frame_compare.cli_entry.runner.run`:
  - Assert exit code `0` when fake runner returns `RunResult(success=True)`.
  - Assert exit code `5` when fake runner returns `RunResult(success=False)`.
...
```bash
UV_CACHE_DIR=./.uv_cache uv run --no-sync python scripts/validate_spec_anchors.py .agent-workflow/runs/2026-02-04__p6-8__run/plan-v1.md
```
...
Then write:
- .agent-workflow/runs/2026-02-04__p6-8__run/plan-review-v1.md
