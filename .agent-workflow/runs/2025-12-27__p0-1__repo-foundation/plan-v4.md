...
> **Revision:** v4 — Addresses FAIL items from plan-review-v3.md (dependency-groups, git prerequisites, lockfile stability check)
...
3. `git diff --exit-code -- uv.lock` fails → Lockfile unstable between syncs
4. `pyright --warnings` reports errors → Return to Planning to fix type issues
5. `ruff check .` reports errors → Return to Planning to fix lint issues
6. `pytest -q` fails → Return to Planning to fix test issues
7. Run-artifact validators fail → Fix artifact structure before proceeding
...
- [ ] **AC-2:** GIVEN `uv.lock` is staged WHEN `uv sync --group dev --frozen` is run followed by `git diff --exit-code -- uv.lock` THEN both commands exit 0 (lockfile unchanged)
...
# ─── Step 2: Stage lockfile for stability check ─────────────────────────────
git add uv.lock
# Pass criteria: exit 0

# ─── Step 3: Verify lockfile stability ───────────────────────────────────────
uv sync --group dev --frozen
# Pass criteria: exit 0

git diff --exit-code -- uv.lock
# Pass criteria: exit 0 (no diff = lockfile unchanged)
...
# ─── Step 8: Run-artifact hygiene validators ─────────────────────────────────
...
8. **Dependency groups vs extras:** This plan uses `[dependency-groups]` (PEP 735), NOT `[project.optional-dependencies]`. This is required for `uv sync --group dev` to work correctly.

9. **Allowed generated files:** The only generated artifact allowed beyond listed files is `uv.lock`. This file is created by `uv sync --group dev` and must be committed.

10. **No other additional files:** Do not create any files not listed in this plan. Specifically:
...
11. **Author email:** Replace `tristan@example.com` with the actual email if known, otherwise leave as-is.

12. **Verification order:** Run verification commands in the exact order listed. Step 1 must succeed before Step 2, and so on. No conditionals, no fallbacks.

13. **Git init:** Step 0 includes `git init` which is idempotent — safe to run even if repo already initialized.
...
Read file: .agent-workflow/runs/2025-12-27__p0-1__repo-foundation/plan-v4.md
...
Write file: .agent-workflow/runs/2025-12-27__p0-1__repo-foundation/plan-review-v4.md
