...
> **Revision:** v2 — Addresses FAIL items from plan-review-v1.md
...
1. `uv sync --group dev --frozen` fails → Check Python version (must be 3.13+) and lockfile existence
2. `pyright --warnings` reports errors → Return to Planning to fix type issues
3. `ruff check .` reports errors → Return to Planning to fix lint issues
4. `pytest -q` fails → Return to Planning to fix test issues
5. Run-artifact validators fail → Fix artifact structure before proceeding
...
### 9. `tests/e2e/test_cli_version.py` [NEW]
...
## 2025-12-27 — Phase 0 Foundation Decisions
...
- [ ] **AC-1:** GIVEN `pyproject.toml` exists WHEN `uv sync --group dev --frozen` is run THEN dependencies install successfully and `.venv/` is created (exit 0)
...
- [ ] **AC-4:** GIVEN Pyright is configured in strict mode WHEN `.venv/bin/pyright --warnings` is run THEN exit code is 0 with 0 errors
- [ ] **AC-5:** GIVEN Ruff is configured WHEN `.venv/bin/ruff check .` is run THEN exit code is 0 with 0 errors
- [ ] **AC-6:** GIVEN pytest is configured WHEN `.venv/bin/pytest -q` is run THEN exit code is 0 and both `test_cli_version_*` tests pass
- [ ] **AC-7:** GIVEN CLI entry point exists WHEN `UV_CACHE_DIR=./.uv_cache uv run --no-sync frame-compare version` is run THEN exit code is 0 and stdout is exactly `frame-compare 0.1.0\n`
- [ ] **AC-8:** GIVEN run artifacts exist WHEN run-artifact validators are executed THEN both exit 0
...
```bash
# Step 0: Prerequisites (must pass before continuing)
uv --version
python --version  # Must show 3.13+

# Step 1: Install dependencies (creates lockfile if missing)
...
# Step 6: Run-artifact hygiene validators
...
6. **Python version:** This project requires Python 3.13+. Ensure `uv` uses Python 3.13.

7. **No additional files:** Do not create any files not listed in this plan. Specifically:
...
8. **Author email:** Replace `tristan@example.com` with the actual email if known, otherwise leave as-is.

9. **Verification order:** Run verification commands in the exact order listed. `uv sync` must succeed before other commands can run.

10. **Lockfile note:** If `uv.lock` does not exist, the first `uv sync --group dev --frozen` will fail. In that case, run `uv sync --group dev` (without `--frozen`) once to create the lockfile, then proceed with `--frozen` for subsequent runs.
...
Write file: .agent-workflow/runs/2025-12-27__p0-1__repo-foundation/plan-review-v2.md
