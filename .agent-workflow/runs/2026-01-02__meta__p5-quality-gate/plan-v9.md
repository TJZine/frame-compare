...
- **Fixed SSOT code block formatting** (per plan-review-v9):
  - Indented `#` comments in section 5.2 code blocks to prevent validator truncation
- **Removed hardcoded VK_ICD_FILENAMES** — omit entirely (let Mesa auto-discover)
- **Kept `-Dopengl=disabled`** — libplacebo build remains headless
...
> [!IMPORTANT]
> The following parity gaps are **explicitly deferred** to separate follow-up RUN_IDs.
...
- Traceability audit (missing E2E tests) → `YYYY-MM-DD__docs__traceability-audit`
...
#### Builder stage — libplacebo meson flags (lines 105-112)
...
> [!NOTE]
> Keep `-Dopengl=disabled` to ensure headless operation; only enable Vulkan.

#### Builder stage — add glslang-tools (around line 36)
...
#### Runtime stage — add Vulkan runtime packages (lines 141-150)
...
> [!IMPORTANT]
> Do NOT set `VK_ICD_FILENAMES`. Mesa lavapipe auto-discovers the correct ICD file for the container's architecture (amd64 or arm64).
...
**Replace `core.placebo.Tonemap()` call block:**
...
3. Keep `-Dopengl=disabled` for headless libplacebo operation
4. Runtime fallback is mandatory: if libplacebo fails at runtime, `_apply_libplacebo` returns `None`
...
**Artifacts:** plan-v1 through plan-v9, impl-v1, impl-v2, verify-v1 through verify-v3
```
...
### 1. Spec Anchor Validation (STOP gate)
...
6. **Keep `-Dopengl=disabled`** — headless libplacebo operation
7. **No VK_ICD_FILENAMES** — let Mesa auto-discover for architecture portability
8. **Remove marker filter** — Docker tests run `tests/integration/` + `tests/vs/` without markers
9. **Rebuild Docker after changes** — `docker compose build frame-compare-test`
...
3. Read file: .agent-workflow/runs/2026-01-02__meta__p5-quality-gate/plan-review-v9.md
...
- Section 5.2: Indented code block comments to fix spec anchor validation

## Spec Anchor STOP Gate Required

Run: `UV_CACHE_DIR=./.uv_cache uv run --no-sync python scripts/validate_spec_anchors.py .agent-workflow/runs/2026-01-02__meta__p5-quality-gate/plan-v9.md`
Must exit 0 before approving.
...
Write file: .agent-workflow/runs/2026-01-02__meta__p5-quality-gate/plan-review-v10.md
