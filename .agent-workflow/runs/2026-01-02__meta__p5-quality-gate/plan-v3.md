...
- **Added signature bullets** for helper functions (`_vs_needs_mock`, `_vs_spec_available`) to pass `validate_spec_anchors.py`
- **Removed ambiguous scope language** ("comprehensive testing improvements for long-term reliability") from Changes Since plan-v1
- **Added exact SSOT heading** "3.2 Conftest Organization" to Spec Anchors
...
Helper functions introduced to fix `find_spec` ValueError issue:
...
**Determinism:** Conversion preserves exact semantics; both methods return pixel sequences.
...
**Root cause:** On macOS with partially-initialized vapoursynth stub package, `find_spec` raises `ValueError: vapoursynth.__spec__ is not set` instead of returning `None`.

**Function signature:** `_vs_needs_mock() -> bool`
...
**Function signature:** `_vs_spec_available() -> bool`
...
**Issue:** Only runs tests in `tests/integration/`. The `vs_required` marker tests in `tests/vs/` are not included.

**Change to include `tests/vs/`:**
...
This ensures `tests/vs/test_integration.py::test_vs_integration_smoke` (marked `@pytest.mark.vs_required`) runs inside Docker where VapourSynth is installed.
...
- RUN_ID: `2026-01-02__meta__p5-quality-gate`
- Artifact versions: plan-v1, verify-v1, plan-v2, plan-review-v2, plan-v3, impl-v1, verify-v2
- Scope: Fix Phase 5 Quality Gate blockers
- SSOT edits: None (contract regeneration only)
...
1. **Contract regeneration first** — Run the generator command before any code changes; commit regenerated files separately if needed
2. **PIL compatibility** — Use try/except pattern to support Pillow < 14 and >= 14; do NOT pin Pillow version
3. **find_spec helper** — Wrap in a local helper function to avoid cluttering module-level code with large try/except
...
6. **No SSOT spec changes** — This run only fixes test infrastructure, not runtime code
...
4. Read file: .agent-workflow/runs/2026-01-02__meta__p5-quality-gate/plan-review-v2.md
...
Write file: .agent-workflow/runs/2026-01-02__meta__p5-quality-gate/plan-review-v3.md
