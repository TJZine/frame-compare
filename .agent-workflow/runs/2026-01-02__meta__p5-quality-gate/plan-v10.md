...
- **Fixed pseudocode indentation bug** (per plan-review-v10):
  - RGBS conversion now runs after successful `try` block, not inside `except`
- **Added explicit Docker libplacebo success test** (per plan-review-v10):
  - New test `test_libplacebo_tonemap_succeeds_in_docker` asserts `_apply_libplacebo` returns non-None
  - This ensures Vulkan backend works (not just that fallback masks failure)
- **Updated acceptance criteria** for mechanically checkable libplacebo success
...
|---|-------|----------|--------|
| 1-5 | Original blockers | BLOCKER | ✅ FIXED (impl-v1/v2) |
| 6 | libplacebo 16-bit input | BLOCKER | 🔴 NEW |
| 7 | No Vulkan in Docker | BLOCKER | 🔴 NEW |
| 8 | No runtime fallback | MEDIUM | 🔴 NEW |

## Scope

- [x] Fixes 1-5: Already implemented
- [ ] Fix 6: Convert to RGB48 before `core.placebo.Tonemap()` (per SSOT 5.2)
- [ ] Fix 7: Enable Vulkan in Dockerfile with Mesa lavapipe
- [ ] Fix 8: Implement runtime fallback when libplacebo fails (per SSOT 5.3)
- [ ] Expand Docker test coverage + add libplacebo success test

---

## Out of Scope (Follow-up RUN_IDs)

- `enable_tonemap` config wiring → `YYYY-MM-DD__feat__auto-tonemap-wiring`
- `use_vspreview` integration → `YYYY-MM-DD__feat__vspreview-integration`
- Traceability audit → `YYYY-MM-DD__docs__traceability-audit`
...
#### Change 2: Update input conversion to RGB48
...
> [!IMPORTANT]
> The RGBS conversion line is **outside** the except block — it runs only when Tonemap succeeds.
...
def test_libplacebo_tonemap_succeeds_in_docker():
    ...
    proving the Vulkan/lavapipe backend is functional. If this test passes,
    the Docker image's libplacebo integration is confirmed working.
    """
    ...
    # Skip if libplacebo not available (should not happen in Docker)
    ...
    assert result is not None, (
        ...
        "Check Mesa lavapipe installation in Docker image."
    )

    # Verify result is a VideoNode
    assert isinstance(result, vs.VideoNode), "Expected VideoNode result"
```
...
2. Dockerfile enables Vulkan via Mesa lavapipe (no explicit VK_ICD_FILENAMES)
3. Keep `-Dopengl=disabled` for headless libplacebo
4. Runtime fallback: `_apply_libplacebo` returns `None` on failure
5. Docker gate must prove libplacebo succeeds (new test `test_libplacebo_tonemap_succeeds_in_docker`)
```
...
- [ ] GIVEN Docker build WHEN `docker compose build` THEN libplacebo compiles with Vulkan
...
- [ ] GIVEN Docker integration WHEN `test_libplacebo_tonemap_succeeds_in_docker` THEN `_apply_libplacebo` returns non-None (proves Vulkan works)
- [ ] GIVEN Docker integration WHEN `verify_docker_integration.sh` THEN zero skips, zero failed
...
**Pass criteria:** Exit 0, coverage ≥ 80%.
...
- `test_libplacebo_tonemap_succeeds_in_docker` passes (proves Vulkan backend works)

---

## Notes for Coding Agent

1. **RGBS conversion is OUTSIDE except block** — runs only on Tonemap success
2. **New test is critical** — `test_libplacebo_tonemap_succeeds_in_docker` must pass in Docker
3. **No VK_ICD_FILENAMES** — let Mesa auto-discover
4. **Keep `-Dopengl=disabled`** — headless operation
5. **Rebuild Docker after changes** — `docker compose build frame-compare-test`
...
3. Read file: .agent-workflow/runs/2026-01-02__meta__p5-quality-gate/plan-review-v10.md
4. Read file: Dockerfile

## Spec Anchor STOP Gate Required

Run: `UV_CACHE_DIR=./.uv_cache uv run --no-sync python scripts/validate_spec_anchors.py .agent-workflow/runs/2026-01-02__meta__p5-quality-gate/plan-v10.md`

## Your Task

Validate plan using 9-point checklist. Verify pseudocode indentation is correct. Confirm libplacebo success test is mandated. Produce Plan Review Report.

## Output

Write file: .agent-workflow/runs/2026-01-02__meta__p5-quality-gate/plan-review-v11.md
