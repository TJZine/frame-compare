...
- **Fixed `ast` test logic** (Review Item #1): Rewrote test to correctly verify `TYPE_CHECKING` guard instead of allowing any top-level `if`
- **Cleaned INPUTS header** (Review Item #2): Only references current artifacts
...
**Test to add:** `test_no_toplevel_vapoursynth_import`

**Test implementation (exact, mechanically correct):**
    ...
    for node in ast.iter_child_nodes(tree):
        ...
        if isinstance(node, ast.ImportFrom) and node.module == "vapoursynth":
            raise AssertionError(
                f"Top-level 'from vapoursynth import' at line {node.lineno}"
            )
        # Top-level If block
...
- [ ] GIVEN `.venv/bin/pytest -v tests/analysis/test_metrics.py::test_no_toplevel_vapoursynth_import` WHEN run THEN passes
...
- Add the **exact** `test_no_toplevel_vapoursynth_import` test as specified above
- The `__all__` list should be sorted alphabetically
...
Read file: .agent-workflow/runs/2025-12-29__p2-5__analysis-integration/plan-v5.md
...
Write file: .agent-workflow/runs/2025-12-29__p2-5__analysis-integration/plan-review-v5.md
