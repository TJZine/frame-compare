...
> **Revision:** v4 — Addresses FAIL items from plan-review-v3.md (.venv ordering: removed from prerequisites, sync runs before Python binary check)
...
1. `uv sync --group dev` fails → Check pyproject.toml syntax and Python version
2. YAML syntax validation fails → Fix workflow file locally
3. CI job fails on PR → Diagnose and fix the specific job locally, then push fixes
4. Any linting/type-check/test jobs that pass locally but fail in CI → Check for environment differences
...
# ─── Step 1: Sync dependencies (creates .venv, updates uv.lock) ──────────────
uv sync --group dev
# Pass criteria: exit 0, .venv/ created, uv.lock updated
...
15. **.venv creation order:** Step 1 runs `uv sync --group dev` which creates `.venv/`. The `.venv/bin/python` validation happens AFTER sync, not before. No conditional logic — sync always runs first.
...
Read file: .agent-workflow/runs/2025-12-27__p0-4__ci-pipeline/plan-v4.md
...
Write file: .agent-workflow/runs/2025-12-27__p0-4__ci-pipeline/plan-review-v4.md
