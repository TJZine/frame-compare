...
1. **Loader selection explicit:** Match SSOT plugin detection - check `hasattr(core.lsmas, 'LWLibavSource')` not just `hasattr(core, 'lsmas')`
2. **Mock class for slicing:** Replaced `SimpleNamespace` with proper `MockClip` class that implements `__getitem__` for slicing
3. **Spec anchor validation added:** Added plan-artifact validation command to verification
4. **Verbatim anchors:** Section format matches validator expectations
...
- `test_detect_hdr_uses_defaults_for_missing_props` — Empty props → `color_primaries=2, transfer=2, matrix=2, is_hdr=False`
...
Write file: .agent-workflow/runs/2025-12-29__p3-2__video-loading/plan-review-v3.md
