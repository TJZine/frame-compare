...
- **[Edit 1]** Updated `test_save_preset_apply_restores_defaults` to use `monkeypatch.chdir(tmp_path)` so that `apply_preset()` finds presets via the relative `DEFAULT_PRESETS_DIR` path.
- **[Edit 2]** Updated acceptance criteria to clarify that `apply_preset()` loads from the default presets directory (`config/presets` relative to CWD).
...
| `test_save_preset_apply_restores_defaults` | Apply preset restores full config (uses monkeypatch.chdir) |
...
**Exact implementation for preset tests:**
...
- [x] GIVEN `save_preset()` then `apply_preset()` with CWD set to workspace root WHEN applied to default config THEN restored config equals original (defaults fill missing optional keys)
...
7. **`apply_preset()` uses relative path:** `DEFAULT_PRESETS_DIR = Path("config/presets")` is relative to CWD. Tests use `monkeypatch.chdir(tmp_path)` to ensure isolation.

8. **STOP triggers:** If `TomlConfigSettingsSource` import fails or signature differs, STOP and return to Planning.
...
Read file: .agent-workflow/runs/2025-12-28__p1-1__config-module/plan-v7.md
...
Write file: .agent-workflow/runs/2025-12-28__p1-1__config-module/plan-review-v7.md
