...
- **SSOT Updated**: Added 0-frame clip behavior to `analysis-module.md`:
  - Section 3.1: Raises include "OR if reference clip has 0 frames"
  - Section 4.1: "Empty Clip Handling" subsection — raise MetricsCalculationError (FC-4002) immediately
  - Section 4.2: "Empty Clip Handling" subsection — same behavior mirrored
- Tests for empty clip are now SSOT-anchored
...
**Edge case tests (SSOT-anchored per 4.1/4.2 Empty Clip Handling):**

- `test_calculate_luminance_empty_clip_raises_error` — 0-frame clip raises `MetricsCalculationError` with `code == "FC-4002"`
- `test_calculate_motion_empty_clip_raises_error` — 0-frame clip raises `MetricsCalculationError` with `code == "FC-4002"`
...
- [ ] GIVEN 0-frame clip WHEN `_calculate_luminance` called THEN raises `MetricsCalculationError (FC-4002)`
- [ ] GIVEN 0-frame clip WHEN `_calculate_motion` called THEN raises `MetricsCalculationError (FC-4002)`
...
- **Empty clip handling**: Check `clip.num_frames == 0` and raise `MetricsCalculationError("Empty clip", ...)` immediately.
...
Read file: .agent-workflow/runs/2025-12-29__p2-2__metrics-calculation/plan-v3.md
...
Write file: .agent-workflow/runs/2025-12-29__p2-2__metrics-calculation/plan-review-v3.md
