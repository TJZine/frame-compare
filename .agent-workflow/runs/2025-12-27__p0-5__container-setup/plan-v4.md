...
- **Fix 1:** Build `zimg` from pinned `sekrit-twc/zimg` release tarball with SHA-256 verification (Bookworm lacks 3.0.5)
- **Fix 2:** Install Cython via pip for VapourSynth R73 + Python 3.13 compatibility
- **Fix 3:** Build L-SMASH from pinned `l-smash/l-smash` release tarball with SHA-256 verification (Bookworm lacks `liblsmash-dev`)
- **Fix 4:** Pin L-SMASH-Works to tag `20230716` and patch SSE2 includes/paths for ARM builds
- **Fix 5:** Add `curl`, `ca-certificates`, `python3-jinja2`, and `libvulkan-dev` to satisfy libplacebo build tooling/stubs
- **Fix 6:** Pin vs-placebo to a commit and clone with submodules (required by build)
- **Fix 7:** Pin ffms2 to an FFmpeg 5-compatible commit (Bookworm provides FFmpeg 5; tag v2.40 fails to link)
- **Fix 8:** Runtime stage installs only `ffmpeg` + `libxxhash0` and copies source-built libs from builder for consistency
...
- Documentation updates outside the container baseline itself (README, etc.) — tracked separately; baseline pin changes must be kept in sync with `deployment.md` and VS baseline references
