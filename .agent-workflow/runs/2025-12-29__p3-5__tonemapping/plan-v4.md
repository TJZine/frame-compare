...
- **SSOT updated:** Section 5.2 now defines exact RGBS conversion rule with already-RGBS no-op condition
- **SSOT updated:** Section 5.2 now defines unified `contrast_recovery` expression with clamping (`0 max 1 min`)
- **SSOT updated:** Section 5.2 now defines exact `std.Expr` invocation shape for RGB planes
- **SSOT updated:** Section 5.3 now explicitly references Section 5.2 rules (no divergence)
...
- `test_get_preset_settings_all_presets_exist` (parametrized)
...
- `test_tonemap_presets_have_correct_values` (parametrized)
...
UV_CACHE_DIR=./.uv_cache uv run --no-sync python scripts/validate_spec_anchors.py .agent-workflow/runs/2025-12-29__p3-5__tonemapping/plan-v4.md
...
Write file: .agent-workflow/runs/2025-12-29__p3-5__tonemapping/plan-review-v4.md
