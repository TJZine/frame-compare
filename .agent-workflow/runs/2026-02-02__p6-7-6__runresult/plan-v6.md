...
INPUTS:
  - .agent-workflow/runs/2026-02-02__p6-7-6__runresult/plan-v5.md
...
- Declares this run blocked on run-artifact hygiene (not plan content): the directory contains an invalid `impl-v1.md`.
- Notes the plan-churn STOP condition: at `plan-v6`, further plan revisions will not remediate the failing run-artifact gate; fix the stale artifact instead.
...
## STOP: Plan Churn Guardrail (Non-Remediating Revisions)

This run has reached `plan-v6`. Per workflow guardrails, further plan rewrites are not an effective response to the current failing gate. The blocker is an invalid existing artifact file in the run directory. Fix run-artifact hygiene first, then proceed with Verification/Review.
...
- [ ] Add focused unit tests for `RunResult` defaults, default factories, and immutability
...
```bash
UV_CACHE_DIR=./.uv_cache uv run --no-sync python scripts/validate_spec_anchors.py .agent-workflow/runs/2026-02-02__p6-7-6__runresult/plan-v6.md
...
3. Rollback is a straightforward revert of the `RunResult` type, the package export, and the unit test file.
...
Read file: .agent-workflow/runs/2026-02-02__p6-7-6__runresult/plan-v6.md
...
Write file: .agent-workflow/runs/2026-02-02__p6-7-6__runresult/plan-review-v6.md
