...
INPUTS:
  - .agent-workflow/runs/2026-02-02__p6-7-6__runresult/plan-v3.md
...
- Makes the run-directory hygiene precondition explicit and actionable (includes the exact invalid file and a concrete removal step).
- Treats the failing `validate_run_artifacts.py` gate as a hard STOP until the stale artifact is removed or corrected by its owning agent.
...
This plan cannot modify prior artifacts (write constraints). Before proceeding, the human orchestrator must take one of the following actions:

1. Remove the stale invalid artifact (recommended for local unblocking):

   ```bash
   rm -f .agent-workflow/runs/2026-02-02__p6-7-6__runresult/impl-v1.md
   ```

2. Alternatively, have the owning agent regenerate a corrected artifact version and remove the invalid one.
...
```bash
UV_CACHE_DIR=./.uv_cache uv run --no-sync python scripts/validate_spec_anchors.py .agent-workflow/runs/2026-02-02__p6-7-6__runresult/plan-v4.md
...
Read file: .agent-workflow/runs/2026-02-02__p6-7-6__runresult/plan-v4.md
...
Write file: .agent-workflow/runs/2026-02-02__p6-7-6__runresult/plan-review-v4.md
