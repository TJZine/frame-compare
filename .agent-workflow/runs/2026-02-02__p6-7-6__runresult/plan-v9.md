...
INPUTS:
  - .agent-workflow/runs/2026-02-02__p6-7-6__runresult/plan-v8.md
...
- Reconfirms this is blocked on run-artifact hygiene (invalid `impl-v1.md`), not on plan content.
- Escalates the plan-churn STOP: do not request additional plan revisions for this gate failure; remove or correct the invalid artifact instead.
...
This run has reached `plan-v9`. Additional plan rewrites are non-remediating for the current failing gate (caused by an invalid existing artifact file). Fix run-artifact hygiene first; do not request additional plan revisions for this same gate failure.

## Run-Directory Hygiene (Precondition / STOP)

The run directory currently fails this gate:
...
Observed failure (from autopilot): `.agent-workflow/runs/2026-02-02__p6-7-6__runresult/impl-v1.md` frontmatter is missing `INPUTS` list entries.
...
Until `validate_run_artifacts.py` passes for the run directory, do not advance phases (STOP).
...
```bash
UV_CACHE_DIR=./.uv_cache uv run --no-sync python scripts/validate_spec_anchors.py .agent-workflow/runs/2026-02-02__p6-7-6__runresult/plan-v9.md
...
Read file: .agent-workflow/runs/2026-02-02__p6-7-6__runresult/plan-v9.md
...
Write file: .agent-workflow/runs/2026-02-02__p6-7-6__runresult/plan-review-v9.md
