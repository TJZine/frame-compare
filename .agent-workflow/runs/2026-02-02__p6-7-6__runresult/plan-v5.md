...
INPUTS:
  - .agent-workflow/runs/2026-02-02__p6-7-6__runresult/plan-v4.md
...
- Clarifies the artifact hygiene resolution: `impl-v2.md` exists and has valid frontmatter; remove the stale invalid `impl-v1.md` so `validate_run_artifacts.py` can pass.
- Treats repeated plan revisions as non-remediating for this gate; further plan changes will not fix the run-directory validator until the invalid artifact is removed or corrected.
...
Observed failure (from autopilot): `impl-v1.md` frontmatter is missing `INPUTS` list entries.
...
1. Remove the stale invalid artifact (recommended since `impl-v2.md` exists and is well-formed):

   ```bash
   rm -f .agent-workflow/runs/2026-02-02__p6-7-6__runresult/impl-v1.md
   ```

2. Or, have the owning agent regenerate a corrected replacement for `impl-v1.md` and then remove the invalid one.
...
```bash
UV_CACHE_DIR=./.uv_cache uv run --no-sync python scripts/validate_spec_anchors.py .agent-workflow/runs/2026-02-02__p6-7-6__runresult/plan-v5.md
...
Read file: .agent-workflow/runs/2026-02-02__p6-7-6__runresult/plan-v5.md
...
Write file: .agent-workflow/runs/2026-02-02__p6-7-6__runresult/plan-review-v5.md
