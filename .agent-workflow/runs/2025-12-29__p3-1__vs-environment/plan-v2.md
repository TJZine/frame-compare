...
- **SSOT updated:** Added `VapourSynthNotFoundError` (FC-2001) to vs-module.md section 3.1 and 6
- **Error classes explicit:** Missing VS import → `VapourSynthNotFoundError`; core failure → `VapourSynthError`
- **Types fully specified:** Added exact dataclass field types and defaults from SSOT
- **conftest.py change concrete:** Adding `mock_vs` fixture per testing-strategy.md (marker already in pyproject.toml)
- **DefaultVSLoader.load() behavior:** Raises `SourceLoadError` (typed) instead of `NotImplementedError`
- **importlinter.ini exact layers:** Verbatim layer order specified
- **STOP instruction added:** Notes for Coding Agent
...
- `is_vapoursynth_available() -> bool` — Try import + core creation, return True/False
...
- `test_is_vapoursynth_available_no_vs_returns_false` — Patch `importlib.import_module` to raise `ModuleNotFoundError`, assert returns `False`
- `test_ensure_vs_environment_missing_vs_raises_not_found_error` — Patch VS import failure, verify `VapourSynthNotFoundError` raised with code `FC-2001`
- `test_ensure_vs_environment_core_failure_raises_vs_error` — Patch VS import success + core failure, verify `VapourSynthError` raised with code `FC-2002`
- `test_detect_plugins_returns_dict_with_expected_keys` — Use `mock_vs` fixture with mocked Core, verify dict has keys: lsmas, libplacebo, bestsource, ffms2
- `test_detect_plugins_missing_plugin_returns_false` — Mock Core without namespace attr, verify `{"lsmas": False, ...}`
- `test_require_plugin_missing_raises_error` — Mock Core without plugin, verify `PluginNotFoundError` raised with code `FC-2003`
...
Note: `frame_compare.vs` inserted between `analysis` and `config` (analysis may import vs; vs may import config/utils/errors).
...
- [ ] GIVEN a VS Core with lsmas plugin WHEN `detect_plugins(core)` is called THEN `{"lsmas": True, ...}` is returned
...
1. **Import guard pattern:** Use try/except for VS availability:

   ```python
   try:
       import vapoursynth as vs
       vs.core  # Validate core creation
   except (ImportError, ModuleNotFoundError):
       # VapourSynthNotFoundError
   except Exception as e:
       # VapourSynthError with details
   ```

2. **Singleton Core:** `DefaultVSLoader._core` is a **class variable**, not instance
3. **Plugin namespace aliases:** lsmas → check both `core.lsmas` and `core.lw`
4. **TYPE_CHECKING guard:** Import `vapoursynth as vs` only under `TYPE_CHECKING`
5. **Error imports:** `from frame_compare.errors import VapourSynthNotFoundError, VapourSynthError, PluginNotFoundError, SourceLoadError`
6. **Test mocking:** Patch at module level (`frame_compare.vs.env`) not globally
...
Write file: .agent-workflow/runs/2025-12-29__p3-1__vs-environment/plan-review-v2.md
