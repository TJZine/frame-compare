...
- **Added explicit `compute_cache_key` encoding algorithm**: Deterministic encoding with `|` delimiter, sorted paths, SHA-256
- **Added invalidation-trigger tests**: `test_compute_cache_key_changes_on_path_change`, `..._size_change`, `..._mtime_change`
...
def compute_cache_key(video_paths: list[Path], config: AnalysisConfig) -> str:
    """
    Generate deterministic hex SHA-256 cache key.

    Encoding rules:
    1. Sort video_paths by str(path) for order-independence
    2. For each path p:
       stat = p.stat()
       h.update(f"{p}|{stat.st_size}|{stat.st_mtime}".encode("utf-8"))
    3. Append config fields with "|" delimiter:
    ...
    5. Return h.hexdigest() (64-char lowercase hex)
    """
    h = hashlib.sha256()
...
**Other functions:** As specified in plan-v4.
...
```bash
UV_CACHE_DIR=./.uv_cache uv run --no-sync python scripts/validate_spec_anchors.py .agent-workflow/runs/2025-12-29__p2-4__cache-io/plan-v5.md
...
- **If SSOT ambiguity encountered**: STOP and return to Planning with CHANGES REQUIRED
...
Write file: .agent-workflow/runs/2025-12-29__p2-4__cache-io/plan-review-v5.md
