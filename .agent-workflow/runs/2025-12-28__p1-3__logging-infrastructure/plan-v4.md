...
- **Spec Anchors fix:** Removed `###` prefixes; now use heading title text only (e.g., `"4.3 Logging + Correlation IDs"`).
- **Verification Commands fix:** Added spec-anchor validation command for this plan artifact.
- **DECISIONS version fix:** Updated to reference plan-v4/plan-review-v3.
- **Test imports fix:** Added explicit `from structlog.testing import ReturnLogger` import.
...
from frame_compare.utils.logging import configure_logging, new_run_id, get_run_id
```
...
**Behavior-based filtering tests:**
...
- RUN_ID: `2025-12-28__p1-3__logging-infrastructure` + artifact versions (plan-v4, plan-review-v3)
...
- Verification gates: pyright, ruff, pytest, spec-anchor validation
...
```bash
# Spec anchor validation (must pass before implementation)
...
3. **Behavior-based filtering:** Use `structlog.wrap_logger` with `ReturnLogger()` (from `structlog.testing`) to verify filtering without private internals.
...
Read file: .agent-workflow/runs/2025-12-28__p1-3__logging-infrastructure/plan-v4.md
...
Write file: .agent-workflow/runs/2025-12-28__p1-3__logging-infrastructure/plan-review-v4.md
