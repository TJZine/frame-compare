...
- Updated SSOT `render-module.md` to make the tonemap failure policy contract-aligned:
  - VS-missing + tonemap-required now fails fast via `VapourSynthNotFoundError (FC-2001)` (no FC-4004 custom hint/message).
  - Added deterministic HDR probing rule (`probe_is_hdr_ffprobe`) for when VS is unavailable/not used.
  - Updated `render_screenshots(...)` public signature in SSOT to include `config: ConfigSchema`.
  - Updated `render_screenshots` loading strategy and `§7.2 Integration Tests` expectations/markers.
- Updated plan to enumerate all in-repo `render_screenshots(...)` call sites that must change with the signature update.
- Updated planned test markers to match SSOT `render-module.md` “7.2 Integration Tests”.
- Updated verification commands to include full quality gates (`pyright` + `ruff`) and cover all modified files.
...
- [ ] Add integration tests (mocked boundaries; no real VS/FFmpeg required by default).
...
- "1.4.1 Gating Rule (Deterministic)" (added deterministic non-VS HDR probe rule: `probe_is_hdr_ffprobe`)
...
- When VS load fails:
  - For `renderer="auto"` and `config.color.enable_tonemap=True`, call `probe_is_hdr_ffprobe(path)`:
    - If HDR: re-raise the original VS failure (no FFmpeg fallback).
    - If SDR: fallback to FFmpeg Path-based rendering.
  - For `renderer="ffmpeg"` and `config.color.enable_tonemap=True`, call `probe_is_hdr_ffprobe(path)`:
    ...
    - If SDR: proceed with FFmpeg rendering.
...
**Required changes:**
- Update all calls to `render_screenshots(...)` to pass a `config: ConfigSchema` argument per SSOT.
...
**Required changes:**
- Update the call to `render_screenshots(...)` to pass a `config: ConfigSchema` argument per SSOT.

### 6. `tests/render/test_tonemap_wiring.py` (NEW)

**Purpose:** Integration tests for tonemap gating scenarios (mocked module boundaries; no real VS/FFmpeg required by default).

**Tests required (spec-anchored; names as in SSOT):**
...
- [ ] GIVEN HDR source + enable_tonemap=True + VS missing + renderer="auto" WHEN `render_screenshots(...)` is called THEN raises `VapourSynthNotFoundError (FC-2001)`
- [ ] GIVEN HDR source + enable_tonemap=True + VS missing + renderer="ffmpeg" WHEN `render_screenshots(...)` is called THEN raises `VapourSynthNotFoundError (FC-2001)`
...
UV_CACHE_DIR=./.uv_cache uv run --no-sync python scripts/validate_spec_anchors.py .agent-workflow/runs/2026-01-04__p6-5__tonemap-wiring/plan-v2.md
...
- Keep HDR probing logic deterministic and isolated (single `ffprobe` JSON query; stable field mapping per SSOT).
- Keep tests pure-Python by mocking `DefaultVSLoader.load` and `probe_is_hdr_ffprobe`; do not require real VS/FFmpeg by default.
...
## Output
Write file: .agent-workflow/runs/2026-01-04__p6-5__tonemap-wiring/plan-review-v2.md
