...
Edits required by `plan-review-v1.md` (2026-02-05):

1. Locked a deterministic CLI invocation model for README examples (activated venv; `frame-compare` on PATH).
2. Locked the API docs generator contract (exit codes, check semantics, constant handling).
3. Locked the module list and ordering for API docs generation (explicit list below).
4. Added a minimal unit-test plan for the generator (no external deps).
5. Added rollback guidance.
6. Removed ambiguity (“as appropriate”) from README flags and from docstring enforcement rules.
7. Added a non-importing (AST-based) generator approach to avoid optional dependency import side effects (VapourSynth).
...
- Phase 7.2 tasks (coverage changes, performance work, or “fix all lint” unrelated to docstrings)
- Contract edits under `docs/OPUS_REBUILD_FRAME_COMPARE/contracts/`
- Any behavior changes to the runtime pipeline (docstrings/docs/tools only)
...
Rationale: examples remain copy/paste runnable from any directory after activation (including the workspace dir),
without requiring absolute paths or repo-relative `.venv/bin` paths.

### B) Public API definition for docstrings (enforced)

Docstring completeness is enforced for exports (by kind) from the module list in the API docs generator section:
...
### D) Exit code contract for the generator script (enforced)
...
- Optional flags (must default to the repo behavior):
  - `--project-root <path>` (defaults to repo root)
  - `--output <path>` (defaults to `<project-root>/docs/api.md`)

**Entry point (signature requirement; do NOT list this as a signature bullet):**

Include a `main()` function that returns an `int` exit code and is used by `__main__`.
...
- Use `tmp_path` to create a minimal fake `src/` tree with a package that mirrors the generator’s expectations:
  - One module that exports:
    - a function with a docstring
    - a class with a docstring
    - a constant assignment (no docstring; should be allowed)
  - One module with a missing docstring for an exported function/class (should fail with exit code 3)
- Run the generator in-process by calling its `main(argv=...)` (or equivalent CLI parsing entrypoint).
...
- Exported constants are exempt from docstring enforcement but should have nearby module-level comments only when
  it meaningfully clarifies intent (no “comment spam”).
...
The following callables MUST be used as the authoritative CLI surfaces for README alignment and docstring quality.
These signatures are present in the CLI SSOT and are used by the Spec Anchors gate.

- `run(...) -> None` — CLI entrypoint (see CLI spec)
- `wizard() -> None` — CLI entrypoint (see CLI spec)
- `doctor(json_output: bool) -> None` — CLI entrypoint (see CLI spec)
...
Read file: .agent-workflow/runs/2026-02-04__p7-1__readme-md-with-usage-examples/plan-v2.md
...
4. Read file: .agent-workflow/runs/2026-02-04__p7-1__readme-md-with-usage-examples/plan-review-v1.md
...
Write file: .agent-workflow/runs/2026-02-04__p7-1__readme-md-with-usage-examples/plan-review-v2.md

If verdict is APPROVED, confirm:
- Verdict: APPROVED
- Implementation Agent Decision Points Remaining: NONE

If verdict is CHANGES REQUIRED, specify concrete edits for a `plan-v3.md` revision.
