...
- **SSOT updated** (`services-module.md` Section 3.2):
  - Added injected `client: httpx.AsyncClient` param to `lookup_tmdb` and `resolve_metadata`
  - Defined `api_key=None` → return `None` without request
  - Defined API key format validation (32 hex chars) → raise `TmdbError`
  - Defined deterministic parser selection: Anitopy first for `[` prefixed filenames
  - Defined fallback: filename stem when parsers fail
  - Defined `prompt_callback` invalid index → raise `MetadataError("invalid selection index")`
- Updated public signatures to match SSOT
- Added exact mocked TMDB JSON payloads for tests
- Added test for invalid callback index
- Replaced verification commands with canonical gates
...
**Movie response (for `test_lookup_tmdb_returns_metadata`):**
...
**TV response (for media_type coverage):**
...
| `test_lookup_tmdb_api_key_none` | `config.api_key=None` | Returns `None`, no HTTP request made |
...
5. **Test Mocking:** Use `respx` (already in dev dependencies):

   ```python
   respx_mock.get(url__startswith="https://api.themoviedb.org/3/search/multi").mock(...)
   ```

6. **Verify no-request for api_key=None:** In `test_lookup_tmdb_api_key_none`, assert `respx_mock.calls.call_count == 0`.
...
Write file: .agent-workflow/runs/2026-01-02__p5-2__metadata-service/plan-review-v2.md
