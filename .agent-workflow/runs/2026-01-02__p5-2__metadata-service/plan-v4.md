...
- Fixed fixture placement ambiguity: `async_client` fixture defined inline in `tests/services/test_metadata.py` (no separate conftest.py)
...
**Fixture (defined inline at top of `tests/services/test_metadata.py`):**
...
> **NOTE:** Do NOT create a separate `tests/services/conftest.py`. Define this fixture inline in the test file.
...
6. **Fixture Location:** Define `async_client` fixture inline at top of `tests/services/test_metadata.py`. Do NOT create `tests/services/conftest.py`.

7. **Test Mocking:** Use `respx` (already in dev dependencies):
...
8. **Verify no-request for api_key=None:** In `test_lookup_tmdb_api_key_none`, assert `respx_mock.calls.call_count == 0`.
...
Write file: .agent-workflow/runs/2026-01-02__p5-2__metadata-service/plan-review-v4.md
