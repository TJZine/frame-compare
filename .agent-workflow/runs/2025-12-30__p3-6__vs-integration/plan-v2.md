...
- **SSOT Update:** Clarified `tonemap`/`apply_tonemap`/`get_preset_settings` exports in `vs-module.md`.
- **Spec Anchors:** Fixed invalid anchor string used in v1; now uses verbatim heading.
- **File List:** Corrected `src/frame_compare/vs/__init__.py` to (MODIFY) and fixed `OUTPUTS` header.
- **Signatures:** Added `## Public API Signatures` section with one-line signatures for the new alias export.
- **Tests:** Fully specified `expected_exports` list and deterministic `__all__` assertion logic in `test_exports.py`.
- **Tests:** Fully specified `test_vs_integration_smoke` logic including skip behavior and blank clip creation.
- **Verification:** Added `validate_run_id`, `validate_run_artifacts`, `validate_spec_anchors` to verification commands.
...
- `test_all_property_is_complete`
  - **Logic:**
    ...
    assert sorted(frame_compare.vs.__all__) == sorted(list(expected))
    ```
...
- `expected` set in `test_exports.py` must match strict list in `__all__`.
...
Write file: .agent-workflow/runs/2025-12-30__p3-6__vs-integration/plan-review-v2.md
