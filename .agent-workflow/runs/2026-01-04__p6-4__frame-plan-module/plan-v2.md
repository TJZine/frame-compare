...
- Updated SSOT `frame-plan-module.md` examples to use `InsufficientFramesError(path=Path("<frame-plan>"), count=<available>, required=<requested>)` under “4.3 Complete Algorithm” and “5. Error Handling”.
- Updated SSOT `frame-plan-module.md` code fences to avoid `# ...` at column 1 (unblocks `validate_spec_anchors.py` for Algorithm-related anchors).
- Removed the plan-v1 SSOT direction (do not align SSOT to current code); plan-v2 aligns `src/frame_compare/errors.py` to SSOT contracts for FC-3004 before implementing FramePlan.
- Tightened negative-case tests to assert FC-3004 payload details (`path`, `count`, `required`) and deterministic placeholder `Path("<frame-plan>")`.
...
- Update `InsufficientFramesError.__init__` signature to `__init__(self, path: Path, count: int, required: int) -> None` per `errors-module.md` “3.3 Input Errors (FC-3xxx) — Exit Code 4”.
- Update `ErrorContext.message`, `ErrorContext.hint`, and `ErrorContext.details` keys to match SSOT for FC-3004.

### 2. `tests/test_errors.py` (MODIFY)

**Purpose:** Keep the error contract test aligned with FC-3004 signature and add a targeted assertion for the SSOT payload shape.

**Tests required:**
- Update the parametrized constructor call for `InsufficientFramesError` to the (path, count, required) signature.
- Add a focused test asserting `.context.details` includes `path`, `count`, `required` (and does not use `requested` / `available`).

### 3. `src/frame_compare/analysis/frame_plan.py` (NEW)
...
### 4. `src/frame_compare/analysis/__init__.py` (MODIFY)
...
### 5. `tests/analysis/test_frame_plan.py` (NEW)
...
UV_CACHE_DIR=./.uv_cache uv run --no-sync python scripts/validate_spec_anchors.py .agent-workflow/runs/2026-01-04__p6-4__frame-plan-module/plan-v2.md
...
## Output
Write file: .agent-workflow/runs/2026-01-04__p6-4__frame-plan-module/plan-review-v2.md
