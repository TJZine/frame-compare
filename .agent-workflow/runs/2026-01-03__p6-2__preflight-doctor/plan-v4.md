...
- **SSOT clarified**: Added deterministic doctor check ordering and slow.pics probe semantics to `orchestration-module.md` §4.2.
- **NoVideosFoundError completed**: Required `self.path` and `self.patterns` instance attributes for deterministic error introspection.
- **Determinism verified**: Added unit tests for env-var expansion, stable case-insensitive input ordering, and slow.pics probe URL/timeout behavior.
...
- Set required instance attributes:
  - `self.path = path`
...
- **network:** slow.pics reachability (per SSOT §4.2.2), TMDB API key presence (`FRAME_COMPARE_TMDB__API_KEY` or legacy `TMDB_API_KEY`; env var check only)

**Check list (deterministic; spec-anchored):**

`collect_checks()` MUST return checks in this exact order (SSOT §4.2.1), with these exact `DoctorCheck.name` strings:
...
**slow.pics probe (deterministic; spec-anchored):**
...
- `test_resolve_paths_expands_env_vars` — Given config `input_dir="$TEST_ROOT/in"` and env set → resolved path expands env var
...
- `test_prepare_preflight_discovers_inputs_sorted_case_insensitive` — Given files `b.mkv` and `A.mkv` → discovered list is `[A.mkv, b.mkv]`
...
- `test_check_slowpics_uses_expected_url_and_timeout` — Mock `httpx.Client.head` and assert URL `https://slow.pics/` + timeout `5.0`
- `test_check_slowpics_fails_on_http_error_status` — Mock response status `500` → `CheckResult(passed=False)` with non-empty hint
...
- SSOT edits: `orchestration-module.md` §4.2.1 and §4.2.2 define deterministic doctor checks and slow.pics probe
...
4. **NoVideosFoundError** — Update constructor signature to `NoVideosFoundError(path: Path, patterns: list[str] | None = None)`, set `self.path` and `self.patterns`, and pass the discovery patterns list from preflight.
...
9. **Network checks** — slow.pics probe uses SSOT §4.2.2 (URL `https://slow.pics/`, HEAD, timeout `5.0`, pass if status < 400). TMDB check verifies presence of `FRAME_COMPARE_TMDB__API_KEY` or legacy alias `TMDB_API_KEY` (no network call).
...
Read file: .agent-workflow/runs/2026-01-03__p6-2__preflight-doctor/plan-v4.md
...
Write file: .agent-workflow/runs/2026-01-03__p6-2__preflight-doctor/plan-review-v4.md
