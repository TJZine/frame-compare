...
# Implementation Plan: Render Encoders (v3)

## Changes Since plan-v2

- **Error Mapping:** Explicitly defined exception mapping for FFmpeg/probe failures (FileNotFound -> FC-2005, CalledProcess -> FC-2006/FC-4015), and mandated wrapping in `RenderError` for the public API.
- **SSOT Alignment:** Re-anchored to updated `render-module.md` (Section 4.2).
- **Test Specificity:** Added exact assertion requirements for tests (e.g., specific `-ss` calculations, error code checks).
- **Verification:** Refined expectations for `lint-imports` and `pyright` behaviors.
...
- `render_frame(request: RenderRequest, renderer: Renderer = "auto") -> Path`
  ...
  - *Logic:* Dispatch based on `renderer` + `request.clip` type.
...
**Purpose:** Export `render_frame` and `RenderError`.
...
- `test_render_frame_vs_dispatch`: Mock `sys.modules["vapoursynth"]`; assert `_render_vs` called.
- `test_render_frame_ffmpeg_dispatch`: Mock missing vs; assert `_render_ffmpeg` called.
...
- `test_error_wrapping`:
  ...
  - Assert: `render_frame` raises `RenderError` (FC-4004).
  - Assert: `exc.cause` is `FFmpegNotFoundError`.
- `test_probe_fps_logic`:
  - Sim: stdout `24000/1001\n`. Assert returns `23.976...`.
...
- `lint-imports` passes (no new forbidden deps).
- `pyright` passes (no errors in strict mode).
...
Write file: .agent-workflow/runs/2026-01-01__p4-5__encoders/plan-review-v3.md
