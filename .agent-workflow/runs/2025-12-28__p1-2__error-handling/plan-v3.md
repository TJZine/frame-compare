...
- **Fix 1:** Added "Planned Public Signatures" subsection with backticked function signatures.
- **Fix 2:** Updated SSOT `errors-module.md` to clarify `PublishError` as marker base (no FC code, never instantiated).
- **Fix 3:** Made `format_error_console` test assertions fully explicit with concrete example.
- **Fix 4:** Added deterministic `CHANGELOG.md` edit rules.
- **Fix 5:** Reduced plan length by removing duplicate parametrize list (tables are canonical).
...
| `PublishError` | `ServiceError` | Marker base; never instantiated (see updated SSOT) |
...
Parametric test covering all 38 new exception classes. Each test asserts: `error.code` matches expected, `error.name` non-empty, `error.hint` non-empty, `error.context.to_dict()` has required keys.

**ExitCode tests:** Verify enum values and `get_exit_code()` mapping for each category + unknown code.

**Formatting tests:**

- `test_format_error_console_basic` — Assert `"✗ Error [FC-"`, contains message, contains `"Hint:"`, contains `"For more details, run with --verbose"`.
- `test_format_error_console_verbose_with_details` — Use `CacheCorruptionError(Path("/cache"))`. Assert output contains `"Details:"`, `"'path'"`, `"/cache"` (using Python repr, not JSON).
...
UV_CACHE_DIR=./.uv_cache uv run --no-sync python scripts/validate_spec_anchors.py .agent-workflow/runs/2025-12-28__p1-2__error-handling/plan-v3.md

# Optional contract gates
...
Write file: .agent-workflow/runs/2025-12-28__p1-2__error-handling/plan-review-v3.md
